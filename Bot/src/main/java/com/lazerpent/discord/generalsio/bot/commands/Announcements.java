...
public class Announcements {
    ...
    private static int drawName(String name, String teamName, int curLine, int fontSize, double crownScale, int spacing, Graphics2D g2) {
        if (tempCrown == null) {
            try {
        ...
        BufferedImage crown = new BufferedImage(tempCrown.getWidth(), tempCrown.getHeight(), BufferedImage.TYPE_INT_ARGB);
        ...
        if (teamName != null) {
            ...
            centerY = curLine - metrics.getAscent() + metrics.getDescent() - metrics.getStringBounds(name, g2).getCenterY();
        }
    ...
    @Command(name = "tournamentfame", desc = "Display hall of fame for tournament champions", perms = Constants.Perms.MOD)
